...
$builddep -y fedora/SPECS/bufr2json.spec

if [[ $image =~ ^fedora: || $image =~ ^centos: ]]
then
    pkgname=bufr2json-master
    mkdir -p ~/rpmbuild/{BUILD,BUILDROOT,RPMS,SOURCES,SPECS,SRPMS}
    cp fedora/SPECS/bufr2json.spec ~/rpmbuild/SPECS/bufr2json.spec
    git archive --prefix=$pkgname/ --format=tar HEAD | gzip -c > ~/rpmbuild/SOURCES/$pkgname.tar.gz
    rpmbuild -ba --define "srcarchivename $pkgname" ~/rpmbuild/SPECS/bufr2json.spec
else
    autoreconf -ifv
    ./configure
    make check
